...
# Kullanıcıya default şifre oluşturma (Adı + '123')
$passwordString = $firstName.ToLower() + "123"
...
if (-not (Get-LocalUser -Name $username -ErrorAction SilentlyContinue)) {
    New-LocalUser -Name $username -FullName "$firstName $lastName" -Password $password -Description "$group Grubu Üyesi" -AccountNeverExpires $true
    Write-Host "Kullanıcı başarıyla oluşturuldu: $firstName $lastName ($username)"
} else {
    Write-Host "Kullanıcı zaten mevcut: $username"
}

# Hesabı etkinleştirme
Enable-LocalUser -Name $username
Write-Host "$username kullanıcısının hesabı etkinleştirildi."
...
Write-Host "Şifre: $passwordString"
